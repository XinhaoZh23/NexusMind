...
def process_file(self, file_id: str):
    ...
    """
    logger.info(f"Starting file processing task {self.request.id} for file_id: {file_id}")
    
    with Session(engine) as session:
        try:
            # 1. Fetch the file record from the database
            file_record = session.exec(select(File).where(File.id == UUID(file_id))).first()
            if not file_record:
                raise ValueError(f"File record with id {file_id} not found in the database.")
            ...
            try:
                ...
                logger.warning(f"Brain {file_record.brain_id} not found. Creating a new one.")
                brain = Brain(
            ...
            nexus_file = NexusFile(file_name=file_record.file_name, file_path=file_record.s3_path)
            registry = setup_processor_registry()
            processor = registry.get_processor(file_record.file_name)
            if not processor:
                raise ValueError(f"No processor found for file type: {file_record.file_name}")
            ...
            if chunks:
                ...
                logger.info(f"Successfully added {len(chunks)} chunks from {file_record.file_name} to brain {brain.brain_id}.")
            else:
                logger.warning(f"No chunks were created from file {file_record.file_name}.")
            ...
            return {"status": "SUCCESS", "message": f"File {file_record.file_name} processed successfully."}

        except Exception as e:
            logger.error(f"Error processing file_id {file_id} in task {self.request.id}: {e}", exc_info=True)
