    ...
    for filename in os.listdir(MIGRATIONS_DIR):
        if filename.endswith(".py") and filename != "__init__.py":
            ...
            if not upgrade_match or not downgrade_match:
                print(f"  WARNING: Could not properly parse upgrade/downgrade functions in {file_path}. Skipping.")
                continue
            ...
            for enum_name in created_enums:
                drop_type_pattern = re.compile(r"DROP TYPE {}".format(re.escape(enum_name)), re.IGNORECASE)
                ...
                if not found_drop_in_downgrade:
                    print(f"  ERROR: Enum '{enum_name}' is created in upgrade, "
